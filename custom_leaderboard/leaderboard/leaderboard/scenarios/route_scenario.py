...
class RouteScenario(BasicScenario):
    ...
    def __init__(self, world, config, debug_mode=0, criteria_enable=True, ego_vehicle=None):
        ...
        if ego_vehicle is None:
            self.ego_vehicle = self._spawn_ego_vehicle() # 12 ms
        else:
            self.ego_vehicle = self._relocate_ego_vehicle(ego_vehicle)
        ...
        self.build_scenarios(self.ego_vehicle, debug=debug_mode > 0)
    ...
    def _spawn_ego_vehicle(self):
        ...
        ego_vehicle = CarlaDataProvider.request_new_actor('vehicle.lincoln.mkz_2020',
                                                          local_transform,
                                                          rolename='hero', ego_vehicle=True)
        if not ego_vehicle:
            return
    ...
    def _relocate_ego_vehicle(self, ego_vehicle):
        elevate_transform = self.route[0][0]
        # Local transform because route should be used as read only.
        local_transform = carla.Transform(elevate_transform.location, elevate_transform.rotation)
        local_transform.location.z += 0.5

        ego_vehicle.set_transform(local_transform)

        # The car can still have forces applied to it from the last tick. Set them to 0
        ...
        CarlaDataProvider.update_ego_vehicle_register()
