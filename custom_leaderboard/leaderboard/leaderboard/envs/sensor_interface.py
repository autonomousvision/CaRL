        ...
        while self._run_ps:
            if self._callback is not None:
                ...
                if current_time - latest_time > (1 / self._reading_frequency) \
                        or (first_time and GameTime.get_frame() != 0) or (current_time - latest_time < 0):
    ...
    def get_data(self, frame):
        ...
        try:
            ...
            while len(data_dict.keys()) < len(self._sensors_objects.keys()):
                ...
                # TODO figure out which sensor is lagging / is last?
                sensor_data = self._data_buffers.get(True, self._queue_timeout)

                if sensor_data[1] != frame:
                    continue

                # print(f'Received sensor: {sensor_data[0]}, Time: {t.tocvalue():4f}')
