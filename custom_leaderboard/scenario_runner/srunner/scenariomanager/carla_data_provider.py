...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
    ...
    def request_new_actor(model, spawn_point, rolename='scenario', autopilot=False,
                          ...
                          attribute_filter=None, tick=True, ego_vehicle=False):
        """
        ...
        if ego_vehicle:
            CarlaDataProvider._ego_vehicle = actor
    ...
    def cleanup():
        ...
        if CarlaDataProvider._ego_vehicle is not None and CarlaDataProvider._ego_vehicle.is_alive:
            batch.append(DestroyActor(CarlaDataProvider._ego_vehicle))
    ...
    def cleanup_route():
        ...
        # Do not destroy ego vehicle. We move it instead of recreating it across scenes
    ...
    @staticmethod
    def update_ego_vehicle_register():
        """
        Add new actor to dictionaries
        If actor already exists, throw an exception
        """
        if CarlaDataProvider._ego_vehicle is not None:
            actor = CarlaDataProvider._ego_vehicle
            transform = CarlaDataProvider._ego_vehicle.get_transform()
            with CarlaDataProvider._lock:
                if CarlaDataProvider._ego_vehicle in CarlaDataProvider._actor_velocity_map:
                    raise KeyError(
                        "Vehicle '{}' already registered. Cannot register twice!".format(actor.id))
                else:
                    CarlaDataProvider._actor_velocity_map[actor] = 0.0
            ...
            print("Error. Ego vehicle is None. Should not happen, investigate!")