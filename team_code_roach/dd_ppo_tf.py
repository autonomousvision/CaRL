...
from model.sensor_model import TransFuserPolicy
from cnn_model import PPOPolicy
...
def save(model, optimizer, config, folder, model_file, optimizer_file):
  ...
  # 1. Get the state_dict from the DDP module
  raw_state_dict = model.module.state_dict()

  # 2. Clean out any '_orig_mod.' prefixes injected by sub-module compilation
  clean_state_dict = {k.replace('_orig_mod.', ''): v for k, v in raw_state_dict.items()}

  # 3. Save the clean state_dict
...
def parse_args(config):
  ...
  parser.add_argument('--use_lidar',
                      type=lambda x: bool(strtobool(x)),
                      default=config.use_lidar,
                      nargs='?',
                      const=True,
                      help='Whether to use a Lidar sensor instead of ground truth perception')
  parser.add_argument('--use_occlusion_check',
                      type=lambda x: bool(strtobool(x)),
                      default=config.use_occlusion_check,
                      nargs='?',
                      const=True,
                      help='Whether to only render bounding boxes that are unoccluded')
  parser.add_argument('--use_sensorimotor',
                      type=lambda x: bool(strtobool(x)),
                      default=config.use_sensorimotor,
                      nargs='?',
                      const=True,
                      help='Whether to train a sensorimotor agent')
  parser.add_argument('--image_architecture',
                      type=str,
                      default=config.image_architecture,
                      help='Backbone used in the image branch.')
  parser.add_argument('--lidar_architecture',
                      type=str,
                      default=config.lidar_architecture,
                      help='Backbone used in the lidar branch.')
  parser.add_argument('--num_grad_acc_steps',
                      type=int,
                      default=config.num_grad_acc_steps,
                      help='How many steps to accumulate the gradient for. Real minibatch size is '
                           'total_minibatch_size * grad_acc_steps Should be multiple and smaller than the number '
                           'of steps per epoch.')
  parser.add_argument('--use_bfloat16',
                      type=lambda x: bool(strtobool(x)),
                      default=config.use_bfloat16,
                      nargs='?',
                      const=True,
                      help=' Whether to do forward and backward passes in bfloat16 mixed precision.')
  parser.add_argument('--use_speed_limit',
                      type=lambda x: bool(strtobool(x)),
                      default=config.use_speed_limit,
                      nargs='?',
                      const=True,
                      help=' Whether to input the current speed limit as measurement to the policy.')
  parser.add_argument('--teacher_path',
                      type=none_or_str,
                      nargs='?',
                      default=config.teacher_path,
                      help='Path to teacher model.pth Used for distillation loss')
  parser.add_argument('--forward_kl',
                      type=lambda x: bool(strtobool(x)),
                      default=config.forward_kl,
                      nargs='?',
                      const=True,
                      help='Whether to use forward KL or reverse KL for distillation.')
  parser.add_argument('--start_kl_percentage',
                      type=float,
                      default=config.start_kl_percentage,
                      help='How much percent to do distillation vs RL at the start')
  parser.add_argument('--end_kl_percentage',
                      type=float,
                      default=config.end_kl_percentage,
                      help='How much percent to do distillation vs RL at the end. Interpolation is linear.')
  parser.add_argument('--features_dim',
                      type=int,
                      default=config.features_dim,
                      help='Number of intermediate features used after Vision encoder.')
...
def main():
  ...
  if config.use_dd_ppo_preempt:
    ...
    torch.distributed.init_process_group('nccl',
                                         ...
                                         device_id=torch.device(f"cuda:{args.gpu_ids[local_rank]}"))
  ...
  device = torch.device(f'cuda:{args.gpu_ids[local_rank]}') if torch.cuda.is_available() and args.cuda else torch.device(
      'cpu')
  ...
  agent = TransFuserPolicy(env.single_observation_space, env.single_action_space, config=config).to(device)
  ...
  agent = torch.nn.parallel.DistributedDataParallel(agent,
                                                    device_ids=[args.gpu_ids[local_rank]],
                                                    output_device=args.gpu_ids[local_rank],
                                                    broadcast_buffers=False,
                                                    find_unused_parameters=False)
  print(f'Rank:{rank}, Created DistributedDataParallel')

  # Only compile cnn the bigger model crashes.
  if config.compile_model:
    if config.image_architecture != 'None':
      agent.module.cnn = torch.compile(agent.module.cnn)
      print(f'Rank:{rank} Compiled cnn')
    if config.lidar_architecture != 'None':
      agent.module.lidar_cnn = torch.compile(agent.module.lidar_cnn)
      print(f'Rank:{rank} Compiled lidar_cnn')
  ...
  obs_device = 'cpu' # We have much more RAM than VRAM, so we keep the observations in RAM to increase mini-batch size.
  ...
  obs = {
      ...
      'measurements':
          torch.zeros(
              ...
              device=obs_device, pin_memory=True),
      'value_measurements':
    ...
    'rgb':
      torch.zeros(
        (local_bs_per_env, args.num_envs_per_proc) + env.single_observation_space.spaces['rgb'].shape,
        device=obs_device,
        dtype=torch.uint8, pin_memory=True),
    'lidar':
      torch.zeros(
        (local_bs_per_env, args.num_envs_per_proc) + env.single_observation_space.spaces['lidar'].shape,
        device=obs_device,
        dtype=torch.uint8, pin_memory=True),
    'compass':
      torch.zeros(
        (local_bs_per_env, args.num_envs_per_proc) + env.single_observation_space.spaces['compass'].shape,
        device=obs_device,
        dtype=torch.float32, pin_memory=True),
    'speed':
      torch.zeros(
        (local_bs_per_env, args.num_envs_per_proc) + env.single_observation_space.spaces['speed'].shape,
        device=obs_device,
        dtype=torch.float32, pin_memory=True),
    'gps':
      torch.zeros(
        (local_bs_per_env, args.num_envs_per_proc) + env.single_observation_space.spaces['gps'].shape,
        device=obs_device,
        dtype=torch.float32, pin_memory=True),
    'target_point':
      torch.zeros(
        (local_bs_per_env, args.num_envs_per_proc) + env.single_observation_space.spaces['target_point'].shape,
        device=obs_device,
        dtype=torch.float32, pin_memory=True),
    'target_point_next':
      torch.zeros(
        (local_bs_per_env, args.num_envs_per_proc) + env.single_observation_space.spaces['target_point_next'].shape,
        device=obs_device,
        dtype=torch.float32, pin_memory=True),
  }
  ...
  next_obs = {
      'bev_semantics': torch.tensor(reset_obs[0]['bev_semantics'], device=obs_device, dtype=torch.uint8).pin_memory(),
      'measurements': torch.tensor(reset_obs[0]['measurements'], device=obs_device, dtype=torch.float32).pin_memory(),
      'value_measurements': torch.tensor(reset_obs[0]['value_measurements'], device=obs_device, dtype=torch.float32).pin_memory(),
      'rgb': torch.tensor(reset_obs[0]['rgb'], device=obs_device, dtype=torch.uint8).pin_memory(),
      'lidar': torch.tensor(reset_obs[0]['lidar'], device=obs_device, dtype=torch.uint8).pin_memory(),
      'compass': torch.tensor(reset_obs[0]['compass'], device=obs_device, dtype=torch.float32).pin_memory(),
      'speed': torch.tensor(reset_obs[0]['speed'], device=obs_device, dtype=torch.float32).pin_memory(),
      'gps': torch.tensor(reset_obs[0]['gps'], device=obs_device, dtype=torch.float32).pin_memory(),
      'target_point': torch.tensor(reset_obs[0]['target_point'], device=obs_device, dtype=torch.float32).pin_memory(),
      'target_point_next': torch.tensor(reset_obs[0]['target_point_next'], device=obs_device, dtype=torch.float32).pin_memory(),
  }
  ...
  if config.teacher_path is not None:
    ...
    config_path = pathlib.Path(config.teacher_path).parent / 'config.json'
    with open(str(config_path), 'rt', encoding='utf-8') as f:
      teacher_json_config = f.read()

    loaded_teacher_config = jsonpickle.decode(teacher_json_config)
    # Overwrite all properties that were set in the saved config.
    teacher_config.__dict__.update(loaded_teacher_config.__dict__)

    teacher = PPOPolicy(env.single_observation_space, env.single_action_space, config=teacher_config).to(device)
  ...
  for update in tqdm(range(start_step, num_updates), disable=rank != 0):
    ...
    if config.teacher_path is not None:
      frac = update / num_updates
      current_kl_percentage = (1.0 - frac) * config.start_kl_percentage + frac * config.end_kl_percentage
    ...
    for step in range(0, local_bs_per_env):
      ...
      with torch.no_grad():
        t1.tic()
        # Automatically detect device type for autocast (handles cpu_collect mode)
        autocast_device_type = 'cuda' if 'cuda' in str(device) else 'cpu'
        with torch.autocast(device_type=autocast_device_type, dtype=torch.bfloat16, enabled=config.use_bfloat16):
      ...
      next_obs = {
          'bev_semantics': torch.tensor(next_obs['bev_semantics'], device=obs_device, dtype=torch.uint8).pin_memory(),
          'measurements': torch.tensor(next_obs['measurements'], device=obs_device, dtype=torch.float32).pin_memory(),
          'value_measurements': torch.tensor(next_obs['value_measurements'], device=obs_device, dtype=torch.float32).pin_memory(),
          'rgb': torch.tensor(next_obs['rgb'], device=obs_device, dtype=torch.uint8).pin_memory(),
          'lidar': torch.tensor(next_obs['lidar'], device=obs_device, dtype=torch.uint8).pin_memory(),
          'compass': torch.tensor(next_obs['compass'], device=obs_device, dtype=torch.float32).pin_memory(),
          'speed': torch.tensor(next_obs['speed'], device=obs_device, dtype=torch.float32).pin_memory(),
          'gps': torch.tensor(next_obs['gps'], device=obs_device, dtype=torch.float32).pin_memory(),
          'target_point': torch.tensor(next_obs['target_point'], device=obs_device, dtype=torch.float32).pin_memory(),
          'target_point_next': torch.tensor(next_obs['target_point_next'], device=obs_device, dtype=torch.float32).pin_memory(),
      }
    ...
    # in case of use_dd_ppo_preempt we still count the preempted samples as collected.
    # they differ between ranks but will be replaced by duplicates
    local_processed_samples += local_bs_per_env * world_size * args.num_envs_per_proc
    config.global_step += local_bs_per_env * world_size * args.num_envs_per_proc
    ...
    if config.use_lstm:
      # Figure out the exact number of local samples
      local_samples = num_collected_steps * args.num_envs_per_proc
      local_samples_tensor = torch.tensor([local_samples], dtype=torch.float32, device=device)

      # Get the sum of all samples across the cluster
      global_samples_tensor = local_samples_tensor.clone()
      torch.distributed.barrier()
      torch.distributed.all_reduce(global_samples_tensor, op=torch.distributed.ReduceOp.SUM)
      global_total_samples = global_samples_tensor.item()

      # Calculate the exact loss scaling weight for this specific GPU
      # Due to DDP_PPO preempt each GPU will have a different mini-batch size.
      # We need to reweight the loss to account for that.
      loss_weight = (local_samples * world_size) / global_total_samples
    ...
    b_obs = {key: obs[key][:num_collected_steps].reshape((-1,) + env.single_observation_space.spaces[key].shape)
             for key in obs}
    ...
    if config.use_lstm:
      ...
      actual_batch_size = num_collected_steps * args.num_envs_per_proc
      flatinds = np.arange(actual_batch_size).reshape(num_collected_steps, args.num_envs_per_proc)
    ...
    if args.norm_adv:
      with torch.no_grad():
        # Distributed mean
        advantage_mean = b_advantages.mean()
        torch.distributed.all_reduce(advantage_mean, op=torch.distributed.ReduceOp.SUM)
        advantage_mean = advantage_mean / world_size

        # Distributed std
        advantage_std = torch.sum(torch.square(b_advantages - advantage_mean))
        torch.distributed.all_reduce(advantage_std, op=torch.distributed.ReduceOp.SUM)
        advantage_std = advantage_std / (world_size * torch.numel(b_advantages) - 1)  # -1 is bessel's correction
        advantage_std = torch.sqrt(advantage_std)

        b_advantages = (b_advantages - advantage_mean) / (advantage_std + 1e-8)

    # Number of minibatches must be divisible by gradient accumulation steps
    assert (num_minibatches % config.num_grad_acc_steps == 0)
    agent.train()
    ...
    for latest_epoch in range(args.update_epochs):
      ...
      for start in range(0, total_steps, step_size):
        ...
        t11.tic()
        b_obs_sampled = {key: b_obs[key][mb_inds].to(device, non_blocking=True) for key in obs}
        # Don't need action, so we don't unscale
        torch.cuda.synchronize()
        t11_transfer.append(t11.tocvalue())

        if config.teacher_path is not None:
          with torch.no_grad():
            _, _, _, _, _, _, _, teacher_distribution, _, _, _ = teacher.forward(
              b_obs_sampled,
        ...
        is_update_step = (num_train_inter % config.num_grad_acc_steps == 0)
        # If it's an update step, use a null context (meaning normal DDP sync).
        # Otherwise, use no_sync to prevent communication.
        sync_context = contextlib.nullcontext() if is_update_step else agent.no_sync()
        with sync_context:
          t8.tic()
          # Handle dynamic device detection for autocast inside training loop
          autocast_device_type = 'cuda' if 'cuda' in str(device) else 'cpu'
          with torch.autocast(device_type=autocast_device_type, dtype=torch.bfloat16, enabled=config.use_bfloat16):
            ...
            if config.teacher_path is not None:
              if config.forward_kl:
                distillation_loss = torch.distributions.kl_divergence(teacher_distribution, distribution).sum(1).mean()
              else:
                distillation_loss = torch.distributions.kl_divergence(distribution, teacher_distribution).sum(1).mean()
              loss = (1.0 - current_kl_percentage) * loss + current_kl_percentage * distillation_loss

            if config.use_lstm:
              loss = loss * (1.0 / config.num_grad_acc_steps) * loss_weight
            else:
              loss = loss * (1.0 / config.num_grad_acc_steps)

          t9.tic()
          loss.backward()
          torch.cuda.synchronize()
          t9_backward.append(t9.tocvalue())
        ...
        t10_optimizer.append(t10.tocvalue())
    ...
    if local_rank == 0:
      print(f'Rank:{rank}, Transfer {sum(t11_transfer)}')
      print(f'Rank:{rank}, Forward {sum(t8_forward)}')
      print(f'Rank:{rank}, Backward {sum(t9_backward)}')
      print(f'Rank:{rank}, Optimizer {sum(t10_optimizer)}')
    ...
    if config.teacher_path is not None:
      torch.distributed.all_reduce(distillation_loss, op=torch.distributed.ReduceOp.SUM)
      distillation_loss = distillation_loss / world_size
    ...
    if rank == 0:
      ...
      if config.teacher_path is not None:
        writer.add_scalar('losses/distillation_loss', distillation_loss.item(), config.global_step)
