...
from model.video_resnet import VideoResNet
...
class TransfuserBackbone(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.transformers = nn.ModuleList([
        GPT(n_embd=self.image_encoder.feature_info.info[start_index + 3]['num_chs'],
            config=config,
            lidar_video=self.lidar_video,
            lidar_time_frames=lidar_time_frames[3])])

    if self.lidar_video:
      self.lidar_channel_to_img = nn.ModuleList([
          nn.Conv3d(self.lidar_encoder.feature_info.info[start_index + 3]['num_chs'],
                    self.image_encoder.feature_info.info[start_index + 3]['num_chs'],
                    kernel_size=1)
      ])
      self.img_channel_to_lidar = nn.ModuleList([
          nn.Conv3d(self.image_encoder.feature_info.info[start_index + 3]['num_chs'],
      ...
      self.lidar_channel_to_img = nn.ModuleList([
          nn.Conv2d(self.lidar_encoder.feature_info.info[start_index + 3]['num_chs'],
                    self.image_encoder.feature_info.info[start_index + 3]['num_chs'],
                    kernel_size=1)
      ])
      self.img_channel_to_lidar = nn.ModuleList([
          nn.Conv2d(self.image_encoder.feature_info.info[start_index + 3]['num_chs'],
                    self.lidar_encoder.feature_info.info[start_index + 3]['num_chs'],
                    kernel_size=1)
      ])
  ...
  def forward(self, image, lidar):
    ...
    image_features, lidar_features = self.fuse_features(image_features, lidar_features, 0)
